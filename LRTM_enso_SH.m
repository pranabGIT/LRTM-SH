...
1. forcing data (DJF + 40 lag days) : tropical convection anom @ sp. regions
2. sigmal matrix (DJF all grid points over NH) : geopot. height anom @250 hPa
...
ifl = input ('Reanalysis (1) or Models (2) :: ');

if ifl == 1
    modl = 'Reanalysis';
    idc = input ('Which time period?? 1 for 1979-2016; 2 for 1979-2005 :: ');
    if idc == 1
        yrst = '1979_2016';
    elseif idc == 2
        yrst = '1979_2005';
    end
    % Forcing data: data prepared on UEA-hpc
    fln = ['Proj_',trm,'ONcmap_frc_mat_lag40_DJF_',yrst,'.mat'];
    load (fln)
   
    % Signal: Geo-Pot height, created on UEA-hpc
    if idc == 1
        load NcepZG_sig_mat_DJF_1979_2015SH_NoFilt.mat % zg250 ano over NH / NCEP
    elseif idc == 2
        load NcepZG_sig_mat_DJF_1979_2005SH_NoFilt.mat
    end
end
