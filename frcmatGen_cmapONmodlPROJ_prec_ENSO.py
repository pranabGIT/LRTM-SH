...
#####################################
** (1) loads daily cmap prec anom composite corresponding to DJF of El Nino years
** (2) loads the anom of daily prec from various CMIP models
...
for i in range(m):
        a = prtr[i,:,:]
        #a = scipy.interpolate.griddata(points, prtr[i,:,:].flatten(), (x,y),method='cubic')
...
for iy in range(len(yln)): 
    ...
    if dy == 360.0:
        indst, indend = yrind+9*30+20, yrind+9*30+20+130 # 9*30=270 for until end of Sepember, 20 for October, so that it starts from 21 Oct ~ this gives us a lag of 40 days before DJF
        yrtot = 360
